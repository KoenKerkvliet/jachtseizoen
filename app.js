...
function gameOverScreen() {
  ...
  app.innerHTML = '<header class="game-header"><div class="brand"><span class="brand-badge">↗</span> Jachtseizoen</div><span class="code">' + game.join_code + '</span></header>'
    + '<section class="timer"><small>Spel afgelopen</small><div class="clock">' + (vangersWin ? 'KRACHT OP!' : 'KLAAR!') + '</div></section>'
    + '<section class="card"><h2>' + (vangersWin ? 'De boeven zijn uitgeschakeld' : 'De sessie is beëindigd') + '</h2><p>' + (vangersWin ? 'De foto-hints kwamen te laat. De boevenkracht is helemaal op.' : 'Bedankt voor het spelen.') + '</p><button class="primary" onclick="leaveLobby()">Terug naar start <span>→</span></button></section>';
}
...
function hearts(quarters) {
  ...
  const assets = ['assets/hearts/heart-empty.svg', 'assets/hearts/heart-quarter.svg', 'assets/hearts/heart-half.svg', 'assets/hearts/heart-three-quarter.svg', 'assets/hearts/heart-full.svg'];
